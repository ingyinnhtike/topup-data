...
class RoleRepository extends BaseRepository
{
    ...
    public function getAll($order_by = 'sort', $sort = 'asc')
    {
        return $this->query()
            ...
            ->where(config('client-access.roles_table').'.company_id', auth()->guard('client')->user()->company->id )
            ->get();
    }
    ...
    public function getForDataTable()
    {
        return $this->query()
            ->with('users', 'permissions')
            ->select([
                config('client-access.roles_table').'.id',
                config('client-access.roles_table').'.name',
                config('client-access.roles_table').'.all',
                config('client-access.roles_table').'.sort',
            ])
            ->where(config('client-access.roles_table').'.company_id', auth()->guard('client')->user()->company->id );
    }
    ...
    public function create(array $input)
    {
        ...
        if (! $all) {
            ...
            if (config('client-access.roles.role_must_contain_permission') && count($input['permissions']) == 0) {
        ...
        DB::transaction(function () use ($input, $all) {
            $role = self::MODEL;
            $role = new $role();
            $role->company_id = auth()->guard('client')->user()->company->id ;
    ...
    {
        if (is_numeric(config('client-access.users.default_role'))) {
            return $this->query()->where('id', (int) config('client-access.users.default_role'))->first();
        }

        return $this->query()->where('name', config('client-access.users.default_role'))->first();
    }
}
